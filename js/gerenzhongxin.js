...
//			};
	...
	//===============用户直到鼠标划过改变img的路径========================/
	$('.liucheng').on('mouseenter','li',function(){
		var index = $(this).index()+1;
		$(this).find('img').attr('src','../css/images/lc_'+index+'.png');
		var $ts_a = $(this).find('.ts_a')
		$ts_a.find('span').css({background:'blue'})
		$ts_a.css({color:'blue'})
	}).on('mouseleave','li',function(){
		var index = $(this).index()+1;
		$(this).find('img').attr('src','../css/images/lc'+index+'.png');
		$(this).find('.ts_a').css({color:''});
		var $ts_a = $(this).find('.ts_a')
		$ts_a.find('span').css({background:''})
		$ts_a.css({color:''})
	})
